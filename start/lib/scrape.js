...
const totalFiturr = () => {
  var mytext = fs.readFileSync("./kyy.js").toString()
...
async function Tts(kyy, m, message, lang) {
  try {
...
async function upScale(path, kyy, m, chatId) {
  try {
    ...
    await kyy.sendMessage(chatId, { image: Buffer.from(data) }, {quoted: m})
  } catch (error) {
    console.error(error);
  }
}
...
async function notifyMaxLevel(user, kyy, m) {
